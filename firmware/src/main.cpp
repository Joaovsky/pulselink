...
static void pack(uint8_t o[8], uint8_t node, int16_t t, uint16_t v, uint8_t h) {
    o[0] = node; o[1] = t >> 8; o[2] = t & 0xFF;
    o[3] = v >> 8; o[4] = v & 0xFF; o[5] = h;
...
int main(void) {
    ...
    if (!device_is_ready(can)) { LOG_ERR("CAN device not ready"); return 0; }
    can_start(can);
    ...
    while (true) {
        ...
        temp = static_cast<int16_t>(temp + 250);
        if (temp > 9000) temp = 2000;
        k_sleep(K_MSEC(500));
    }
    return 0;
}
